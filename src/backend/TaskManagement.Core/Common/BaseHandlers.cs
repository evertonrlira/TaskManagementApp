...
{
    protected readonly IApplicationLogger<BaseCommandHandler<TCommand, TResult>> Logger;

    protected BaseCommandHandler(IApplicationLogger<BaseCommandHandler<TCommand, TResult>> logger)
    {
        Logger = logger;
    }
    ...
    public async Task<TResult> HandleAsync(TCommand command)
    {
        ...
        using var scope = Logger.BeginScope("Command Execution", 
            new { CommandType = commandType, Command = command });
        ...
        try
        {
            ...
            stopwatch.Stop();
            Logger.LogPerformance(commandType, stopwatch.Elapsed, 
                new { Success = true, CommandType = commandType });
...
{
    protected readonly IApplicationLogger<BaseQueryHandler<TQuery, TResult>> Logger;

    protected BaseQueryHandler(IApplicationLogger<BaseQueryHandler<TQuery, TResult>> logger)
    {
        Logger = logger;
    }
    ...
    public async Task<TResult> HandleAsync(TQuery query)
    {
        ...
        using var scope = Logger.BeginScope("Query Execution", 
            new { QueryType = queryType, Query = query });
        ...
        try
        {
            ...
            stopwatch.Stop();
            Logger.LogPerformance(queryType, stopwatch.Elapsed, 
                new { Success = true, QueryType = queryType });
